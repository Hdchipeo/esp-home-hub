...
static void print_data_to_console(data_station_t *weather_data)
{
    if (!weather_data)
    {
        ESP_LOGW(TAG, "NULL weather_data pointer!");
        return;
    }

    ESP_LOGI(TAG, "=== Weather Station Data ===");
    ESP_LOGI(TAG, "LDR1        : %d", weather_data->ldr1);
    ESP_LOGI(TAG, "LDR2        : %d", weather_data->ldr2);
    ESP_LOGI(TAG, "Battery     : %.1f%%", weather_data->battery_capacity);
    ESP_LOGI(TAG, "Temperature : %.1f°C", weather_data->temperature);
    ESP_LOGI(TAG, "Rain State  : %s", weather_data->rain_state ? "Dry" : "Raining");
    ESP_LOGI(TAG, "Rain Value  : %d mV", weather_data->rain_traffic);
...
static void handle_recv_callback(const example_espnow_event_recv_cb_t *recv_cb)
{
    ...
    if (recv_cb->data_len == sizeof(data_station_t))
    {
    ...
    else if (recv_cb->data_len == sizeof(light_state_t))
    {
        ESP_LOGI(TAG, "Received light state data");
        light_state_t light_state = *(light_state_t *)recv_cb->data;
        ESP_LOGI(TAG, "Light state: %s", light_state ? "ON" : "OFF");
        light_update_rainmaker(light_state);
    }
    else
    {
        ESP_LOGW(TAG, "Unknown data length: %d", recv_cb->data_len);
    }
...
static esp_err_t espnow_init(void)
{
    ...
    /* Add light peer information to peer list. */
    espnow_add_peer(light_mac_addr, false);
...
void espnow_send_light_state(bool power)
{
    ...
    esp_err_t ret = esp_now_send(light_mac_addr, (uint8_t *)&light_state, data_len);
    if (ret != ESP_OK)
    {
